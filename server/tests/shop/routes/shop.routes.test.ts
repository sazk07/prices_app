...
export const testShopRoutes = () => {
  describe("Shop functional test", () => {
    ...
    it("should return 200 OK as JSON response and list of shops on GET /shops", async () => {
      ...
      const data = (await response.json()) as ShopOutput[];
      assert.deepStrictEqual(response.status, 200);
      assert.deepStrictEqual(data.length, 2);
      assert.deepStrictEqual((data[0] as ShopOutput).shopName, "Test Shop 1");
      assert.deepStrictEqual((data[1] as ShopOutput).shopName, "Test Shop 2");
      assert.deepStrictEqual((data[0] as ShopOutput).shopLocation, "Test Location 1");
      assert.deepStrictEqual((data[1] as ShopOutput).shopLocation, "Test Location 2");
      assert.deepStrictEqual((data[0] as ShopOutput).shopId, 1);
      assert.deepStrictEqual((data[1] as ShopOutput).shopId, 2);
    });
